  ...
  (function () {
    ...
    for (var iv = 0; iv < vLines; iv++) {
      ...
      var mat2 = new THREE.LineBasicMaterial({ transparent: true, opacity: 0.2 });
    ...
    sparkDefs.forEach(function (def, idx) {
      ...
      var lineMat = new THREE.LineBasicMaterial({ transparent: true, opacity: 0.5 });
    ...
    function applyTheme() {
      var lineColor = new THREE.Color(cssColor("--line-strong", "#ccd8e3"));
    ...
    function renderFrame() {
      ...
      sparks.forEach(function (s) {
        s.dots.forEach(function (d, idx) {
          var p = reduceMotion ? (idx * 0.1) : ((t * s.speed - idx * 0.09) % 1 + 1) % 1;
          d.mesh.position.copy(toVec(p * Math.PI * 2, s.v));
        });
      });
